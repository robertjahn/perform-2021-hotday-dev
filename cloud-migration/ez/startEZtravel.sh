...
echolog "************************************************************************"
echolog "Starting EZ travel Start Script @ $START_TIME"
echolog "************************************************************************"
...
# allow log to start (5min), then check to see if started ok
while IFS= read -r LOGLINE || [[ -n "$LOGLINE" ]]; do
    printf '%s\n' "$LOGLINE"
    [[ "${LOGLINE}" == *"easyTravel procedures started successfully"* ]] && echo "easyTravel READY" && break"
done < <(timeout 300 tail -f /tmp/weblauncher.log || echolog "Problem launching EasyTravel")

END_TIME="$(date)"
echolog "************************************************************************"
echolog "EZ Travel Start Script Done"
